...
class FileStore:
    ...
    def hashes_for_repository(
        ...
        rows = self._session.execute(
            select(SourceFile.path, SourceFile.content_sha256).where(
    ...
    def delete_files_not_in(
        ...
        An empty walk must never mean that every indexed file disappeared.
        """

        if not keep_paths:
            logger.warning(
                "Skipping cleanup for %s: walk returned no files",
                repository_id,
            )
            return 0
        ...
        result = self._session.execute(
            delete(SourceFile).where(
                SourceFile.repository_id == repository_id,
                SourceFile.path.notin_(keep_paths),
            )
        )
